...
std::string getCharacterSet(const std::vector<bool>& options) {
    ...
    if (options[0]) {
        characterSet += CHARACTER_SETS[0];
    }
    if (options[1]) {
        characterSet += CHARACTER_SETS[1];
    }
    if (options[2]) {
        characterSet += CHARACTER_SETS[2];
    }
    if (options[3]) {
        characterSet += CHARACTER_SETS[3];
    }
    if (options[4]) {
        characterSet += CHARACTER_SETS[4];
    }
    return characterSet;
}
...
double calculateCombinations(const std::vector<bool>& options) {
    ...
    for (int i = 0; i < options.size(); ++i) {
        if (options[i]) {
            combinations *= CHARACTER_SETS[i].size();
        }
    }
    return combinations;
}
